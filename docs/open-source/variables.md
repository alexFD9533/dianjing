...
当前正式扩展没有服务端环境变量、账号密钥、数据库连接、项目自有 API 或第三方 provider。构建配置位于根 `package.json`、各 workspace `package.json` 和 `apps/dock-extension/vite.config.ts`；项目配置 `.cairn/config.yaml` 只包含项目元数据和本地 Cairn 策略，不含凭证。
...
- 构建前检查 `git diff`、zip 内容和日志，确认没有把本地过程、Cairn、旧版目录或秘密带入公开产物。
